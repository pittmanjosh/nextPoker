...
describe("sortCards", () => {
  test("sorts cards by getPoints", () => {
    const sortedCards = sortCards([threeOfSpades, aceOfSpades, twoOfSpades]);
    expect(sortedCards[0].getPoints()).toBe(14);
    expect(sortedCards[1].getPoints()).toBe(3);
    expect(sortedCards[2].getPoints()).toBe(2);
  });

  test("sorts cards by suit", () => {
  ...
  test("sorts by value.points, then suit", () => {
    ...
    expect(sortedCards[0].getPoints()).toBe(14);
    expect(sortedCards[1].getSuit()).toBe("diamonds");
    expect(sortedCards[1].getPoints()).toBe(14);
    ...
    expect(sortedCards[4].getPoints()).toBe(2);
    expect(sortedCards[5].getSuit()).toBe("diamonds");
    expect(sortedCards[5].getPoints()).toBe(2);
  });
});
